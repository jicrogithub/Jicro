...
import { getFcmToken, requestPushNotificationPermision } from '../../helper/Notification';
import { getData } from '../../helper/LocalStorage';
const Auth = ({ navigation }) => {
  const [auth, setAuth] = useState(null)
  const { verifyUser, verifyServiceProvider, shouldNavigateUser, shouldNavigateServiceProvider, isError } = useAuth()
  ...
  return (
    <View className="h-screen" >
      ...
      <View style={main.shadows} className="h-[45%] w-full bg-white mt-[-20px] shadow-xl rounded-3xl py-8 px-2" >
        ...
        <Seperator text="*" />
        <TouchableOpacity disabled={status} onPress={() => {
      ...
      >
        <View className="h-full w-full p-2" >
          <Text className="text-[#684DE9] font-black text-6xl text-center" >Welcome</Text>
          <Text className="text-gray-600 font-black text-sm text-center">How is Your business Going 🤗?</Text>
          <View className="my-3" >
            <WhatsApp disabled={status} status={status} set={setAuth} setTO={"SPAuth"} bgColor={main.success} text={"Login with WhatsApp"} />
            <Seperator text="OR" />
            <Button func={() => {
              navigation.navigate("_Profile")
          ...
          <View>
            <Text className="text-center text-gray-400 text-[11px]" >By Continuing as Service Provider You are agreeing Our</Text>
            <Text className="text-center text-gray-400 font-black" >Terms and Conditions</Text>
          </View>
        </View>
      </RBSheet>
